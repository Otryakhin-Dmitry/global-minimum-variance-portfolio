...
Among packages devoted to portfolio management the most sophisticated seems to be fPortfolio by RMetrics, Wuertz et al (2014). They formulate the MV portfolio optimization problem as minimizing the portfolio variance 
$$
...
Traditionally, weight-shrunk and custom mean variance portfolios are returned as objects of class MeanVar_portfolio. Ones with shrunk weights have a subclass inheriting from it: MV_portfolio_weights_BDOPS21 and GMV_portfolio_weights_BDPS19. For MeanVar_portfolio methods summary and plot are available.
...
```{r, fig.pos = "!H", fig.width=3.8}
portfolio_BDPS19 <- MVShrinkPortfolio(x=x, gamma=Inf, type='traditional', b=b, beta = 0.05)
summary(portfolio_BDPS19)
plot(portfolio_BDPS19)
```
